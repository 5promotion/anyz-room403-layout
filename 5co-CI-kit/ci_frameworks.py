...
def bars_line(points, w=470, h=175, ylab="", y2lab="", bar_unit="M",
    ...
    points: [(x軸ラベル, 棒の値, 折れ線の値 or None, 棒が信頼できるか bool)]
      ...
      - 棒が信頼できない月は**琥珀の薄塗り＋破線**で描く（空白にすると「データが無い」と誤読されるため）
    ...
    for i, (lab, bv, lv, ok) in enumerate(points):
        ...
        else:   # 要確認の月＝琥珀の薄塗り＋破線。空白にすると「データが無い」と誤読されるため塗る
        ...
        if n <= 9:        # 点が少ないときは値を棒の上に出す
            o.append(_t(x + bwid / 2, y - 4, f"{bv:,.0f}", 9, INK, "middle", not ok))
    # 折れ線（None の区間で切る）
    seg = []
    for i, (lab, bv, lv, ok) in enumerate(points):
        if lv is None:
